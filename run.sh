...
qemu-system-x86_64 \
  ...
  -enable-kvm \
  -netdev tap,id=t0,ifname=qemu_tap0,script=no,downscript=no -device igb,netdev=t0 \
  -netdev tap,id=t1,ifname=qemu_tap1,script=no,downscript=no -device igb,netdev=t1 \
