...
int main(int argc, char **argv)
{
    ...
    /* Else client */
        ...
        if (SSL_connect(ssl) == 1) {
            ...
            // Fill txbuf with "LEAK"
            char msg[] = {'x', 'x', 'L', 'E', 'A', 'K', 'x', 'x'}; 
            int msg_idx = 0;
            ...
            for (int i = 0; i < 100; i++) {
                txlen = (random() & ~0xf) % (MAX_BUF_SIZE/2);
                total_bytes_sent += txlen;
                printf("Packet %d: send %d bytes\n", i, txlen);
                ...
                if (rxlen <= 0) {
                    ...
                    /* Show it */
                    printf("Received: %d bytes\n", rxlen);
                    total_bytes_received += rxlen;
                    for (int j = 0; j < rxlen; j++) {
                        if (msg[2+msg_idx] != txbuf[j])
                            printf("ERROR: (msg[%d]) '%c' != '%c' (txbuf[%d])\n", msg_idx, msg[2+msg_idx], txbuf[j], j);
                        msg_idx = (msg_idx + 1) % 4;
                    }
                }
            }
            ...
            printf("Client exiting, bytes_sent = %d, bytes_received = %d\n", total_bytes_sent, total_bytes_received);
            bzero(msg, 8);
        } else {
...
exit:
    ...
    bzero(rxbuf, MAX_BUF_SIZE);
    bzero(txbuf, MAX_BUF_SIZE);
